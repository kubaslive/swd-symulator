...
const footerEndIdx = app.indexOf(oldFooterEnd, footerIdx);

if (footerIdx !== -1 && footerEndIdx !== -1) {
  app = app.substring(0, footerIdx) + newFooter + app.substring(footerEndIdx + oldFooterEnd.length);
}

// 2. Remove Dziennik Radiowy WIDGET correctly!
const drStrStart = '{/* --- OLD DZIENNIK RADIOWY REMOVED --- */}';
const dzEnd = '          </div>\n        )}\n\n        {/* MAP MODAL */}';

const drIdx = app.indexOf(drStrStart);
const drEndIdx = app.indexOf(dzEnd, drIdx);

if (drIdx !== -1 && drEndIdx !== -1) {
   app = app.substring(0, drIdx) + '\n        {/* MAP MODAL */}' + app.substring(drEndIdx + dzEnd.length);
}
