...
const oldCoordsStart = 'const getCoordinatesForLocation = (locStr) => {';
...
const coordsRegex = /const getCoordinatesForLocation = \(locStr\) => \{[\s\S]*?return \{ x, y \};\n\};/m;
app = app.replace(coordsRegex, newCoords);

// 3. renderMapaGIS update
const oldMapRegex = /const renderMapaGIS = \(\) => \{[\s\S]*?<\!-- END MAPA -->.*?<\/div>\n\s*<\/div>\n\s*\);\n\s*\};/m;
...
// Because the SVG map is complex, I will use indexOf and substring
const mapStart = 'const renderMapaGIS = () => {';
const mapEndStr = '  // Render Procedury Postępowania';
